...
export const MyTicketsPage: React.FC = () => {
  ...
  const handleDownloadTicket = (ticket: any, index: number) => {
    const ticketSummary = [
      'Memora Digital Ticket',
      `Order ID: ${ticket.orderId}`,
      `Ticket ID: ${ticket.orderId}-${index}`,
      `Museum: ${ticket.details?.museumName ?? 'Museum belum tersedia'}`,
      `Ticket Type: ${ticket.details?.ticketType ?? 'General Ticket'}`,
      `Visit Date: ${
        ticket.details?.visitDate
          ? new Date(ticket.details.visitDate as string).toLocaleDateString('id-ID')
    ...
    link.href = url;
    link.download = `${ticket.orderId}-${index}.txt`;
    link.click();
              ...
              <CardContent className="space-y-4">
                ...
                <div className="flex gap-2">
                  ...
                  <Button
                    ...
                    aria-label={`Download ticket ${ticket.orderId}`}
                  >
